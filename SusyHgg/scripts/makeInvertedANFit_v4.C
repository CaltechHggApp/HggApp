...
RooWorkspace* makeInvertedANFit(TTree* tree, TString weightFilename,float forceSigma=-1, bool constrainMu=false, float forceMu=-1) {
  RooWorkspace *ws = new RooWorkspace("ws","");


  TFile weightFile(weightFilename);
  RooWorkspace *weightWorkspace = (RooWorkspace*)weightFile.Get("ws");
  ...
  RooArgSet AIC_constraints("AIC_constraints");
  ...
  for(auto func = bkgPdfList.begin(); func != bkgPdfList.end(); func++) {
    ...
    AIC_constraints.add(*(bres->createHessePdf(bres->floatParsFinal())));
  ...
  RooDataSet* covarData = (RooDataSet*)weightWorkspace->data("weights");
  std::cout << "covarData " << covarData << std::endl;
  TMatrixDSym* covar = covarData->covarianceMatrix();
  std::cout << "covar " << covar << std::endl;
  RooArgSet weights("weights");// = covarData->get();
  RooArgSet weight_means("weights_means");// = covarData->get();
  RooArgSet pdfs("pdfs");

  for(auto t = tags.begin(); t!=tags.end(); t++) {
    RooAbsPdf *p = ws->pdf("b_"+*t);
    pdfs.add(*p);
    std::cout << *t << "  " << p << "  " << *t+"_w" << std::endl;
    weights.add( *(new RooRealVar(*t+"_w","",covarData->mean( *(weightWorkspace->var(*t+"_w"))),0,1)));
    weight_means.add( *(new RooRealVar(*t+"_w_mean","",covarData->mean( *(weightWorkspace->var(*t+"_w"))))));
  }
  std::cout << "building constraint " << std::endl;

  RooMultiVarGaussian weightConstraint("weightConstraint","",RooArgList(weights),RooArgList(weight_means),*covar);

  RooAddPdf b_AIC("b_AIC","",pdfs,weights);

  AIC_constraints.add(weightConstraint);

  b_AIC.fitTo(data,RooFit::Strategy(0),RooFit::Extended(kTRUE),RooFit::Range("sideband_low,sideband_high"),RooFit::ExternalConstraints(AIC_constraints));
  RooFitResult* bres = b_AIC.fitTo(data,RooFit::Strategy(2),RooFit::Save(kTRUE),RooFit::Extended(kTRUE),RooFit::Range("sideband_low,sideband_high"),RooFit::ExternalConstraints(AIC_constraints));
  ...
  RooFitResult* sbres;
  if(constrainMu) {
    AIC_constraints.add(HiggsMassConstraint);
  }
  fitModel.fitTo(data,RooFit::Strategy(0),RooFit::Extended(kTRUE),RooFit::ExternalConstraints(AIC_constraints));
  sbres = fitModel.fitTo(data,RooFit::Strategy(2),RooFit::Save(kTRUE),RooFit::Extended(kTRUE),RooFit::ExternalConstraints(AIC_constraints));
