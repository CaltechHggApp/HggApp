...
RooWorkspace* makeInvertedANFit(TTree* tree, TString weightFilename,float forceSigma=-1, bool constrainMu=false, float forceMu=-1) {
  ...
  for(auto func = bkgPdfList.begin(); func != bkgPdfList.end(); func++) {
    ...
    RooRealVar sigma(tag+"_s_sigma","#sigma",5,0,100);
    if(forceSigma!=-1) {
      sigma.setVal(forceSigma);
      sigma.setConstant(true);
    }
    RooRealVar mu(tag+"_s_mu","#mu",126,120,130);
    if(forceMu!=-1) {
      mu.setVal(forceMu);
      mu.setConstant(true);
    }
    RooGaussian sig(tag+"_sig_model","",mgg,mu,sigma);
    RooRealVar Nsig(tag+"_sb_Ns","N_{signal}",5,0,100);
    RooRealVar Nbkg(tag+"_sb_Nb","N_{background}",100,0,100000);
  ...
  b_AIC.fitTo(data,RooFit::Strategy(0),RooFit::Extended(kTRUE),RooFit::Range("sideband_low,sideband_high"),RooFit::ExternalConstraints(weightConstraint));
  RooFitResult* bres = b_AIC.fitTo(data,RooFit::Strategy(2),RooFit::Save(kTRUE),RooFit::Extended(kTRUE),RooFit::Range("sideband_low,sideband_high"),RooFit::ExternalConstraints(weightConstraint));
  bres->SetName("AIC_b_fitres");
  ws->import(*bres);
  

  RooRealVar sigma("AIC_s_sigma","#sigma",5,0,100);
  if(forceSigma!=-1) {
    sigma.setVal(forceSigma);
    sigma.setConstant(true);
  }
  RooRealVar mu("AIC_s_mu","#mu",126,120,130);
  if(forceMu!=-1) {
    mu.setVal(forceMu);
    mu.setConstant(true);
  }
  RooGaussian sig("AIC_sig_model","",mgg,mu,sigma);
  RooRealVar Nsig("AIC_sb_Ns","N_{signal}",5,0,100);
  RooRealVar Nbkg("AIC_sb_Nb","N_{background}",100,0,100000);
