...
void make1DLimit(TString combine_dir,TString type= "WH",bool blind=true){
  ...
  TGraph obser_r( (275-125)/25 );
  TGraph graph_r( (275-125)/25 );
  TGraphAsymmErrors error_r( (275-125)/25 );
  ...
  for(int m=125;m<=275;m+=25) {
    ...
    cv.SaveAs(combine_dir+"/expected_exclusion_"+type+"_1D.png");
    ...
    cv.SaveAs(combine_dir+"/expected_exclusion_ratio_"+type+"_1D.png");
}
