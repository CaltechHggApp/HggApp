...
RooWorkspace* makeMggFit(TTree* tree,bool doBkg=true, float forceSigma=-1,float forceMu=-1,float forceA1=1,float forceA2=1, float forceF=-1) {
  ...
  TLatex muL(0.62,0.8,Form("#mu = %0.1f #pm %0.2f GeV",mu.getVal(),mu.getError()));
  muL.SetTextSize(0.042);
  muL.SetNDC();

  TLatex sigL(0.62,0.72,Form("#sigma = %0.1f #pm %0.2f GeV",sigma.getVal(),sigma.getError()));
