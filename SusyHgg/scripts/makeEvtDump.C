...
void dumpAllBoxes() {
  ...
  TTree *sel = passAll->CopyTree("pho1_pt>40 && pho2_pt>25 && pho1_pass_iso && pho2_pass_iso && abs(pho1_eta)<1.5 && abs(pho2_eta)<1.5");
...
void splitAllBoxes() {
  ...
  TTree *sel = passAll->CopyTree("pho1_pt>40 && pho2_pt>25 && pho1_pass_iso && pho2_pass_iso && abs(pho1_eta)<1.5 && abs(pho2_eta)<1.5 && mgg>103 && mgg < 160");

  TFile *f1 = new TFile("./DoublePhoton_22Jan2013_Run2012ABCD_v7__HighPt.root","RECREATE");
  TTree *HighPt = sel->CopyTree("ptgg>=110");
  HighPt->Write();
  f1->Close();

  TFile *f2 = new TFile("./DoublePhoton_22Jan2013_Run2012ABCD_v7__Hbb.root","RECREATE");
  TTree *Hbb = sel->CopyTree("ptgg < 110 && (abs(mbb_NearH-125)<15)");
  Hbb->Write();
  f2->Close();

  TFile *f3 = new TFile("./DoublePhoton_22Jan2013_Run2012ABCD_v7__Zbb.root","RECREATE");
  TTree *Zbb = sel->CopyTree("ptgg < 110 && !(abs(mbb_NearH-125)<15) && (abs(mbb_NearZ-91.2)<15)");
  Zbb->Write();
  f3->Close();

  TFile *f4 = new TFile("./DoublePhoton_22Jan2013_Run2012ABCD_v7__HighRes.root","RECREATE");
  TTree *HighRes = sel->CopyTree("ptgg < 110 && !(abs(mbb_NearH-125)<15) && !(abs(mbb_NearZ-91.2)<15) && (pho1_sigEoE<0.015 && pho2_sigEoE<0.015)");
  HighRes->Write();
  f4->Close();

  TFile *f5 = new TFile("./DoublePhoton_22Jan2013_Run2012ABCD_v7__LowRes.root","RECREATE");
  TTree *LowRes = sel->CopyTree("ptgg < 110 && !(abs(mbb_NearH-125)<15) && !(abs(mbb_NearZ-91.2)<15) && !(pho1_sigEoE<0.015 && pho2_sigEoE<0.015)");
  LowRes->Write();
  f5->Close();
